...
import { Descriptions, Modal, Skeleton, Image, Tag, DescriptionsProps, Button, message } from "antd";
...
export default function Page() {
	...
	const handleScan = () => {
		...
		fetch('/api/actionOnQr?secret_key=' + qrData)
		.then((res) => res.json())
		.then((data) => {
			console.log(data);
			if(data.status == "ok") success(data.message)
			...
			if(data.isGuest)
			{
				items = [
					...
					},
					{
						...
						children: <p>{data.user.visit_purpose}</p>,
						span: 2
					},
					{
						key: '7',
			...
			<Modal title="Информация" open={isModalOpen} onOk={handleScan} onCancel={handleCancel}
			        footer={[
						...
						<Button key="submit" type="primary" loading={loading} onClick={handleScan}>
						  Скан {user?.isBreak ? (!user?.user?.is_onBreak ? "на перерыв" : "с перерыва") : (user?.user?.is_entered ? "на выход" : "на вход")}
						</Button>
					  ]}>
				{loading && <Skeleton />}
				{!loading && user &&
				...
				{!loading && user?.user?.photo_path &&
					<Image
						width={400}
						src={`/assets/${user?.user?.photo_path}`}
